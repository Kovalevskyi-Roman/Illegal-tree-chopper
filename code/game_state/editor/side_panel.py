...
class SidePanel:
    def __init__(self, game_state_manager: "GameStateManager", selected_tile: int, game_objects: list[dict[str, Any]],
                 selected_game_object: int, move_game_object: bool, snap_to_grid: bool) -> None:
        self.game_state_manager = game_state_manager
        self.selected_tile = selected_tile
        self.game_objects = game_objects
        self.selected_game_object = selected_game_object
        self.move_game_object = move_game_object
        ...
        self.safe_tile_del_btn: Button = Button(
            ...
            f"Safe del: {self.safe_tile_deleting}",
            font,
            "#efefef"
        )

        self.move_game_object_lbl: Label = Label(f"[M]ove: {self.move_game_object}", font, "#efefef")
        self.snap_to_grid_lbl: Label = Label(f"To [G]rid: {self.snap_to_grid}", font, "#efefef")
    ...
    def update(self, selected_tile: int, game_objects: list[dict[str, Any]],
               selected_game_object: int, move_game_object: bool, snap_to_grid: bool) -> None:
        self.selected_tile = selected_tile
        self.game_objects = game_objects
        self.selected_game_object = selected_game_object
        ...
        self.move_game_object_lbl.update(f"[M]ove: {self.move_game_object}")
        self.snap_to_grid_lbl.update(f"To [G]rid: {self.snap_to_grid}")
        ...
        if self.safe_tile_del_btn.is_active():
            self.safe_tile_deleting = not self.safe_tile_deleting
            self.safe_tile_del_btn.text = f"Safe del: {self.safe_tile_deleting}"
        ...
        # if self.new_game_obj_btn.is_active():
    ...
    def draw(self) -> None:
        ...
        Window.ui_surface.blit(
            TileManager.tile_textures[self.selected_tile],
            [self.x + 16, 16]
        )
        ...
        if self.selected_game_object != -1:
            self.selected_game_obj_name.update(f"Obj: {self.game_objects[self.selected_game_object].get("name")}")
            self.selected_game_obj_name.draw(
                Window.ui_surface,
                [self.x + 16, 112 + TileManager.TILE_SIZE * 2]
            )
            self.selected_game_obj_pos.update(f"Pos: {self.game_objects[self.selected_game_object].get("data").get("position")}")
