...
class DataEditor(GameState):
    ...
    def init(self) -> None:
        self.__editor = self.game_state_manager.GAME_STATES.get(self.game_state_manager.EDITOR)
        self.entry.set_text(  # Превращает игровой объект в отформатированную строку
            json.dumps(self.__editor.game_objects[self.__editor.selected_game_object], indent=4)
        )
    ...
    def update(self, *args, **kwargs) -> None:
        ...
        if pygame.key.get_just_pressed()[pygame.K_ESCAPE]:
            self.__editor.game_objects[self.__editor.selected_game_object] = json.loads(self.entry.get_as_one_line())
